...
def get_energy_pr(collection, rk, collection_key):
    ...
    if 'code_comments_python' in collection_key:
        ...
        for ck in collection_key:
            ...
            blist_regex = ''
            blist = []
            if rk in black_list.keys():
                print('Entering here...')
                if (len(black_list[rk])) > 0:
                    #blist_regex += '{ "$nor": [ '
                    i = 0
                    for bl in black_list[rk]:
                        regex='{ '+ck+": {'$regex': '"+bl+"'}}"
                        regex2={ ck: {'$regex': bl}}
                        blist_regex+=regex
                        blist.append(regex)
                        i+=1
                        #print(str(i)+' <-> '+str(len(black_list[rk])))
                        if len(black_list[rk]) > i:
                            blist_regex+=','
                        #else:
                        #    blist_regex+=']}'
                    print(blist_regex)
...
black_list['.*consum.*'] = ['.*time\sconsum.*',
                 '.*consumer.*',
                 '.*time-consum.*'
                 ]

black_list['.*efficien.*'] = [ '.*coefficient.*',
                              '.*efficient\salgo.*',
                              '.*efficient\sway.*',
                              '.*efficiently.*',
                              '.*efficient\scode.*'
...
black_list['.*wak.*'] = [ '.*.benewake*',
                 '.*nowak.*'
                 ]

black_list['.*amper.*'] = [ '.*ampersand.*',
                 '.*hamper.*'
                 ]

black_list['.*sleep.*'] = [ '.*sleep().*'
                           ]

black_list['.*sustainab.*'] = [ '.*unsustainable.*'
                               ]

black_list['.*green.*'] = [ '.*green\scolor.*',
                 '.*green\slight.*',
                 '.*green\sline.*',
                 '.*green\sdot.*',
                 '.*green\sarrow.*',
...
keys["bitbucket"] = ['pr_title', 
                     'pr_contents', 
                     'pr_comments']
fname["bitbucket"] = 'data/bitbucket_pr_data.json'
"""
# StackOverFlow
...
keys["stackoverflow"] = ['title', 
                         ...
                         'question_code', 
                         'answer_code']
fname["stackoverflow"] = 'data/new_stackoverflow_data.json'
"""
# ROSDiscurse
...
fname["rosdiscurse"] = 'data/ros-discourse_data.json'
...
fname["rosanswers"] = 'data/ros-answers_data.json'
...
fname["prsgit"] = 'data/github-prs_data.json'
# Open Issues
...
fname["issues"] = 'data/new_github-issues_data.json'

# Commits
cols["commit"] = [db.Commits]
keys["commit"] = [ 'Commit Info'
                  ]
fname["commit"] = 'data/commit_data_new.json'
...
fname["repo"] = 'data/repositories_data.json'
"""
##############################
### EXECUTION
#####
